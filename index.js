...
async function getAIResponse(prompt) {
    ...
    for (const api of apis) {
        try {
            ...
            if (result && result.trim().length > 3 && !result.toLowerCase().includes("failed") && !result.toLowerCase().includes("error")) {
                return result.trim();
            }
        } catch (e) {
            continue;
    ...
    return "Désolé, mes sources IA sont saturées. Réessayez plus tard !";
}
